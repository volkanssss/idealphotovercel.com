...
export default function useVintageState() {
  // State management
  const [capturedPhotos, setCapturedPhotos] = useState<string[]>([]);
  const [isCapturing, setIsCapturing] = useState(false);
  const [bgColor, setBgColor] = useState<string>('bg-gradient-to-r from-amber-100 to-yellow-100');
  const [selectedFilter, setSelectedFilter] = useState<DigiboothFilterType>('sepia');
  ...
  const [frameColor, setFrameColor] = useState<FrameColorType>('softYellow');
